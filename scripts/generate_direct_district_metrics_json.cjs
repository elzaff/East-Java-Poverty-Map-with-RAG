...
const csvPath = [
  path.join(__dirname, '../../NOTEBOOK-DATAFINAL/Data-DL/direct_district_all_models_deliverable/direct_district_all_models_metrics_summary.csv'),
  path.join(__dirname, `../../NOTEBOOK-DATAFINAL/Data-DL/${legacyDistrictName}_all_models_deliverable/${legacyDistrictName}_all_models_metrics_summary.csv`),
].find(fs.existsSync);
const outPath = path.join(__dirname, '../public/api/data_direct_district_metrics.json');

if (!csvPath) {
  console.error('Missing Direct District metrics CSV');
  process.exit(1);
}
...
for (let i = 1; i < lines.length; i++) {
  ...
  const is_production = (model === 'MLP' && scenario === 'J3_PCD_IMAGE');
