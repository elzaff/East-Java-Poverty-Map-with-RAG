...
import { Kecamatan, LayerId, TimeseriesDataPoint, ShapValue } from '../types';
import { KECAMATAN_LIST, generateMockTimeseries, generateShapValues, JAWA_TIMUR_REGENCY_COORDINATES } from '../mockData';
...
export function MapStage({
  ...
  const [kecamatanBoundaries, setKecamatanBoundaries] = useState<any>(null);
  ...
  const onMouseMove = useCallback((evt: any) => {
    ...
    if (feature && feature.properties && feature.properties.gid_3) {
      ...
      let kec = KECAMATAN_LIST.find(k => k.gid_3 === gid_3);
      let name_3 = feature.properties.name_3 || (kec && kec.name_3) || '';
      let name_2 = feature.properties.name_2 || (kec && kec.name_2) || '';
      
      if (gid_3.endsWith('-KAB')) {
        const regId = gid_3.replace('-KAB', '');
        const reg = JAWA_TIMUR_REGENCY_COORDINATES.find(r => r.id === regId);
        name_3 = reg ? reg.name : name_3;
        name_2 = 'Kabupaten / Kota';
      }

      const val = feature.properties.val !== undefined ? feature.properties.val : (layerData[gid_3] || 0);
      
      // Look up series or make robust fallback
      let ts = generateMockTimeseries(gid_3).find(d => d.year === year);
      if (!ts) {
        ts = {
          year,
          poverty_risk_score: val,
          estimasi_kemiskinan: val * 12.5,
          prediction_dl: val,
          ...
          ntl: 0.6 - val * 0.4,
          vanui: 0.52 - val * 0.2,
          distress_share: val * 1.1,
          prediction_error: 0.05
        };
      }

      const shap = generateShapValues(gid_3, year);
      setHoveredInfo({
        x: evt.point.x,
        y: evt.point.y,
        name_3,
  ...
  useEffect(() => {
    async function fetchBoundaries() {
      try {
        ...
        const [kec, kab, predKabData] = await Promise.all([
          fetch(`${base}api/kecamatan.json?t=${Date.now()}`).then(r => r.json()),
          fetch(`${base}api/kabupaten.json?t=${Date.now()}`).then(r => r.json()),
          fetch(`${base}api/data_predictions_kabupaten.json?t=${Date.now()}`).then(r => r.json()).catch(() => ({}))
        ]);
        setKecamatanBoundaries(kec);
        setKabupatenBoundaries(kab);
        setPredKab(predKabData);
        ...
        for (const city of cities) {
          ...
          try {
            ...
            if (merged) { merged.properties = { name: city }; cityFeatures.push(merged); }
          } catch {
  ...
  const getColor = useCallback((val: number, layerId: LayerId, minVal: number = 0, maxVal: number = 1) => {
    ...
    // Poverty / risk / distress: kuning (rendah) → merah gelap (tinggi), 10 stops, clip 10%
    ...
    // Diverging: hijau (membaik) → putih (tidak berubah) → merah (memburuk)
    if (layerId === 'poverty_delta') {
      return idx(['#166534','#16a34a','#4ade80','#f0fdf4','#fee2e2','#ef4444','#7c2d12']);
    }
    // Prediction error: biru muda ke ungu
    if (layerId === 'prediction_error') {
      return idx(['#0d0d0d','#1e1b4b','#4338ca','#818cf8','#c7d2fe','#ffffff']);
    }
    // NDVI: coklat kering ke hijau subur
    if (layerId === 'ndvi') {
      return idx(['#7c3a00','#a16207','#ca8a04','#84cc16','#22c55e','#166534']);
    }
    // NDBI (built-up index): abu gelap ke oranye urban
    if (layerId === 'ndbi') {
      return idx(['#0d0d0d','#1c1917','#78350f','#b45309','#f59e0b','#fde68a']);
    }
    // NDWI (water index): krem ke biru laut dalam
    if (layerId === 'ndwi') {
      return idx(['#451a03','#92400e','#38bdf8','#0284c7','#075985','#0c4a6e']);
    }
    // NTL (night time lights): hitam ke kuning terang seperti kota malam
    if (layerId === 'ntl') {
      return idx(['#0d0d0b','#1a1a00','#3d2b00','#f59e0b','#fde047','#ffffff']);
    }
    // VANUI: ungu gelap ke pink terang
    if (layerId === 'vanui') {
      return idx(['#0d0d0d','#2e1065','#6d28d9','#a855f7','#e879f9','#fdf4ff']);
    }
  ...
  const geojsonData = useMemo(() => {
    ...
    // Pre-compute aggregated value per kabupaten from full 663 polygon dataset
    ...
    (kabupatenBoundaries.features as any[]).forEach((feat: any) => {
      ...
      const v = layerData[nameKey] !== undefined ? layerData[nameKey] : layerData[p.gid_3];
    ...
    (kabupatenBoundaries.features as any[]).forEach((feat: any) => {
      ...
      if (viewLevel === 'kecamatan' && p.gid_3) {
        ...
        const isPercentLayer = activeLayer === 'poverty_risk_score' || activeLayer === 'estimasi_kemiskinan';
        const fallback = nlpLayers.includes(activeLayer) ? 0 : (kabVal[p.reg_id] ?? (isPercentLayer ? 15 + (localHash(p.gid_3) % 1000) / 100 : 0.28 + (localHash(p.gid_3) % 40) / 100));
        val = layerData[nameKey] !== undefined
          ? layerData[nameKey]
          : layerData[p.gid_3] !== undefined
            ? layerData[p.gid_3]
            : fallback;
        gid_3 = p.gid_3;
      } else {
        // Kabupaten view: satu blok warna per kabupaten
        const isPercentLayer = activeLayer === 'poverty_risk_score' || activeLayer === 'estimasi_kemiskinan';
        val = kabVal[p.reg_id] ?? (isPercentLayer ? 15 + (localHash(p.reg_id || '') % 1000) / 100 : 0.28 + (localHash(p.reg_id || '') % 40) / 100);
        gid_3 = p.reg_id + '-KAB';
      }
    ...
    // Absolute global ranges for stable cross-year comparison
    ...
    const features = rawFeatures.map(({ feat, val, gid_3, p }) => ({
      ...
      properties: {
        gid_3,
        name_3: viewLevel === 'kabupaten' ? 'Semua Kecamatan' : (p.name_3 || p.name_2),
        name_2: p.name_2,
        val,
  ...
  }, [layerData, activeLayer, selectedGid, cityFilter, getColor, viewLevel, kabupatenBoundaries]);

  // Styling untuk Layer Masking (Fill)
  const fillStyle: any = {
    id: 'kecamatan-fill',
    type: 'fill',
    paint: {
      ...
      'fill-opacity': [
        ...
        0.5 // Default opacity for unselected
      ]
    }
  };

  // Styling untuk Layer Garis Batas (Line)
  const lineStyle: any = {
    id: 'kecamatan-line',
    type: 'line',
    paint: {
  ...
  const onMapClick = useCallback((evt: any) => {
    ...
    if (feature && feature.properties) {
      if (viewLevel === 'kabupaten' && feature.properties.reg_id) {
        onSelectKecamatan(feature.properties.reg_id + '-KAB', feature.properties.name_2, '');
  ...
  return (
    <div className="relative w-full h-full bg-[#0a0a0b] flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        {/* Real City Boundaries fetched from OSM API */}
        {cityBoundaries && (
        ...
        {/* Layer Poligon Mock (Masking Choropleth) */}
        {geojsonData && (
      ...
      {/* Dynamic Hover Pop-up with score & XAI data */}
      {hoveredInfo && (
        <div 
          ...
          style={{ 
            ...
            transform: `translate3d(${
              hoveredInfo.x > window.innerWidth * 0.55 ? '-105%' : '15px'
            }, ${
              hoveredInfo.y > window.innerHeight * 0.55 ? '-105%' : '15px'
            }, 0)`
          }}
        >
          {/* Header */}
          ...
          {/* Poverty risk and main statistics */}
          <div>
            ...
            {viewLevel === 'kabupaten' && (activeLayer === 'estimasi_kemiskinan' || activeLayer === 'poverty_risk_score') && (
              <div className="bg-black/30 border border-white/5 p-2 rounded mb-2 space-y-1.5">
                <div className="flex justify-between items-center text-[10px]">
                  <span className="text-[#f59e0b]/80 uppercase font-bold tracking-wider">ESTIMASI AGGREGATE</span>
                  <span className="font-mono text-[#f59e0b] font-bold">{hoveredInfo.val.toFixed(2)}%</span>
                </div>
                {predKab[hoveredInfo.name_2]?.series?.find((s: any) => s.year === year) && (
                  <>
                    <div className="flex justify-between items-center text-[10px]">
                      <span className="text-[#ec4899]/80 uppercase font-bold tracking-wider">ESTIMASI DIRECT (MUMTAZ)</span>
                      <span className="font-mono text-[#ec4899] font-bold">{predKab[hoveredInfo.name_2].series.find((s: any) => s.year === year).prediction.toFixed(2)}%</span>
                    </div>
                    {predKab[hoveredInfo.name_2].series.find((s: any) => s.year === year).bps !== null && predKab[hoveredInfo.name_2].series.find((s: any) => s.year === year).bps !== 0 && (
            ...
            {hoveredInfo.metrics && (
              <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-[10px]">
                ...
                <div className="flex justify-between border-b border-white/5 pb-0.5 col-span-2">
                  <span className="text-white/40">VANUI</span>
                  <span className="font-mono text-purple-400 font-medium">{hoveredInfo.metrics.vanui.toFixed(3)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1.5 border-t border-white/5 pt-2">
            <h5 className="text-[9px] font-bold text-white/40 uppercase tracking-widest">
              XAI Risk Drivers (SHAP)
            </h5>
            <div className="space-y-1">
              {hoveredInfo.shap.slice(0, 3).map((item, idx) => {
                const isPositive = item.value > 0;
                return (
                  <div key={idx} className="flex items-center justify-between text-[10px] bg-black/20 px-2 py-1 rounded">
                    <span className="text-white/70 truncate max-w-[150px]" title={item.feature}>
                      {item.feature.replace('event_score_', '')}
                    </span>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-white/40 font-mono text-[9px]">
                        ({item.actual_value.toFixed(1)})
                      </span>
                      <span className={
                        isPositive ? "text-red-400 font-mono font-bold text-[9px]" : "text-green-400 font-mono font-bold text-[9px]"
                      }>
      ...
      <div className="absolute top-6 left-6 z-20 pointer-events-none bg-[#1a1a1e]/90 border border-white/10 p-4 rounded shadow-2xl backdrop-blur">
        <h2 className="text-[10px] text-white/50 uppercase tracking-widest mb-1">Active Layer</h2>
        <div className="text-lg font-bold text-white uppercase">{activeLayer.replace(/_/g, ' ')}</div>
             ...
             {Array.from({ length: 10 }).map((_, i) => {
               ...
               return (
                 <div 
                   key={i} 
                   className="flex-1" 
                   style={{ backgroundColor: getColor(v, activeLayer, 0, 1) }} 
                 />
               );
             })}
          </div>
